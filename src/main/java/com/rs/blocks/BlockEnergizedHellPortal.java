...
import com.rs.Repo;
import com.rs.world.EnergizedHell.EnergizedHellTeleporter;
...
public class BlockEnergizedHellPortal extends BlockPortal
{
    public BlockEnergizedHellPortal()
    {
        super();
        setBlockName("portalEnergizedHell");
    ...
    public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
    {
        if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP)))
        {
            ...
            else if (player.dimension != Repo.dimensionID)
            {
                player.timeUntilPortal = 10;

                player.mcServer.getConfigurationManager().transferPlayerToDimension(player, Repo.dimensionID, new EnergizedHellTeleporter(mServer.worldServerForDimension(Repo.dimensionID)));
            }
            else
            {
                player.timeUntilPortal = 10;
                player.mcServer.getConfigurationManager().transferPlayerToDimension(player, 0, new EnergizedHellTeleporter(mServer.worldServerForDimension(1)));
            }
        }
    }
    ...
    public boolean func_150000_e(World par1World, int par2, int par3, int par4)
    {
        ...
        else
        {
            ...
            for (l = 0; l < 2; ++l)
            {
                for (i1 = 0; i1 < 3; ++i1)
                {
                    par1World.setBlock(par2 + b0 * l, par3 + i1, par4 + b1 * l, Repo.energizedHellPortal, 0, 2);
                }
            }
