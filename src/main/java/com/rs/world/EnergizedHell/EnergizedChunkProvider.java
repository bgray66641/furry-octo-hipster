package com.rs.world.EnergizedHell;

import com.rs.Repo;
import com.rs.world.GenerateCastle;
...
public class EnergizedChunkProvider implements IChunkProvider
{
    ...
    private GenerateCastle castle = new GenerateCastle();
    double[] noiseData1;
    double[] noiseData2;
    double[] noiseData3;
    double[] noiseData4;
    ...
    public EnergizedChunkProvider(World par1World, long par2)
    {
        ...
        this.ExculsivityNoiseGen = new NoiseGeneratorOctaves(this.hellRNG, 4);
        this.NoiseGen6 = new NoiseGeneratorOctaves(this.hellRNG, 10);
        this.NoiseGen7 = new NoiseGeneratorOctaves(this.hellRNG, 16);

        NoiseGenerator[] noiseGens = { NoiseGen1, NoiseGen2, NoiseGen3, ExculsivityNoiseGen, NoiseGen6, NoiseGen7 };
        noiseGens = TerrainGen.getModdedNoiseGenerators(par1World, this.hellRNG, noiseGens);
        this.NoiseGen1 = (NoiseGeneratorOctaves) noiseGens[0];
        this.NoiseGen2 = (NoiseGeneratorOctaves) noiseGens[1];
        this.NoiseGen3 = (NoiseGeneratorOctaves) noiseGens[2];
        this.ExculsivityNoiseGen = (NoiseGeneratorOctaves) noiseGens[3];
        this.NoiseGen6 = (NoiseGeneratorOctaves) noiseGens[4];
        this.NoiseGen7 = (NoiseGeneratorOctaves) noiseGens[5];
    }
    ...
    public void func_147418_b(int p_147418_1_, int p_147418_2_, Block[] p_147418_3_)
    {
        ...
        byte b0 = 64;
        double d0 = 0.03125D;
        this.ExclusivityNoise = this.ExculsivityNoiseGen.generateNoiseOctaves(this.ExclusivityNoise, p_147418_1_ * 16, p_147418_2_ * 16, 0, 16, 16, 1, d0 * 2.0D,
                d0 * 2.0D, d0 * 2.0D);
        ...
        for (int k = 0; k < 16; ++k)
        {
            for (int l = 0; l < 16; ++l)
            {
                int i1 = (int) (this.ExclusivityNoise[k + l * 16] / 3.0D + 3.0D + this.hellRNG.nextDouble() * 0.25D);
                int j1 = -1;
                Block block = Repo.energizedHellStone;
                Block block1 = Repo.energizedHellStone;
                ...
                for (int k1 = 127; k1 >= 0; --k1)
                {
                    ...
                    if (k1 < 127 - this.hellRNG.nextInt(5) && k1 > this.hellRNG.nextInt(5))
                    {
                        ...
                        {
                            if (block2 == Repo.energizedHellStone)
                            {
                                if (j1 == -1)
                                {
                                    if (i1 <= 0)
                                    {
                                        block = null;
                                        block1 = Repo.energizedHellStone;
    ...
    private double[] initializeNoiseField(double[] par1ArrayOfDouble, int par2, int par3, int par4, int par5, int par6, int par7)
    {
        ...
        double d0 = 684.412D;
        double d1 = 2053.236D;
        this.noiseData4 = this.NoiseGen6.generateNoiseOctaves(this.noiseData4, par2, par3, par4, par5, 1, par7, 1.0D, 0.0D, 1.0D);
        this.noiseData5 = this.NoiseGen7.generateNoiseOctaves(this.noiseData5, par2, par3, par4, par5, 1, par7, 100.0D, 0.0D, 100.0D);
        this.noiseData1 = this.NoiseGen3.generateNoiseOctaves(this.noiseData1, par2, par3, par4, par5, par6, par7, d0 / 80.0D, d1 / 60.0D, d0 / 80.0D);
    ...
    {
        return "EnergizedHellRandomLevelSource";
    }
